...
public final class PartyCommand extends MythicCommand {
    ...
    public void usage(@NotNull Player player) {
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#F529BEParty Commands"));
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#FFFFFF/party create &7- start a new party with you as leader"));
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#FFFFFF/party invite <player> &7- invite a player to your party"));
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#FFFFFF/party join <id> &7- accept an invite using the party id"));
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#FFFFFF/party leave &7- leave your current party"));
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#FFFFFF/party disband &7- disband the party (leader only)"));
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#FFFFFF/party info &7- show your party's members and roles"));
        player.sendMessage(net.mythicpvp.suite.hex.MythicHex.colorize("&#FFFFFF/party chat <message> &7- send a message in party chat"));
    }
    ...
    public void create(@NotNull Player player) {
        ...
        player.sendMessage(messages.component(
                ...
                "&#9CFF9CParty created. Party id: &#FFFFFF%id%&#9CFF9C.",
    ...
    public void invite(@NotNull Player player, @NotNull String targetName) {
        ...
        if (party == null || !party.leader().equals(player.getUniqueId())) {
            player.sendMessage(messages.component(
                    ...
                    "&#FF8A8AYou must lead a party to do that."));
            return;
        }
        ...
        target.sendMessage(messages.component(
                ...
                "&#FFFFFF%sender% &#9CFF9Cinvited you to party &#FFFFFF%id%&#9CFF9C. Use &#FFFFFF/party join %id%&#9CFF9C.",
                Map.of("sender", player.getName(), "id", Long.toString(party.id()))));
        player.sendMessage(messages.component(
                "messages.social.party-invite-sent",
                "&#9CFF9CParty invite sent to &#FFFFFF%target%&#9CFF9C.",
    ...
    public void join(@NotNull Player player, long partyId) {
        if (!social.joinParty(partyId, player.getUniqueId())) {
            player.sendMessage(messages.component(
                    ...
                    "&#FF8A8AUnable to join that party."));
            return;
        }
        player.sendMessage(messages.component(
                "messages.social.party-joined",
                "&#9CFF9CJoined party &#FFFFFF%id%&#9CFF9C.",
    ...
    public void leave(@NotNull Player player) {
        if (!social.leaveParty(player.getUniqueId())) {
            player.sendMessage(messages.component(
                    ...
                    "&#FF8A8AYou are not in a party."));
            return;
        }
        player.sendMessage(messages.component(
                "messages.social.party-left",
                "&#9CFF9CYou left the party."));
    }
    ...
    public void info(@NotNull Player player) {
        ...
        player.sendMessage(messages.component(
                ...
                "&#FFFFFFParty &#D2D8E0%id% &#FFFFFFhas &#D2D8E0%count% &#FFFFFFmembers.",
    ...
    public void chat(@NotNull Player player, @NotNull String[] words) {
        ...
        if (words.length == 0) {
            player.sendMessage(messages.component(
                    ...
                    "&#FF8A8AUsage: &#FFFFFF/party chat <message>"));
            return;
        }
