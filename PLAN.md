...
- ✅ **Punishment menu text → YAML** — `PunishmentMenuText` reads operator-overridable strings from `menus.yml` (titles, button names, click hints, state labels). Falls back to historical hard-coded defaults when the file is missing or partial; rank editor strings still pending.
