...
- ✅ **Friends, party, mail, offline rewards** — `/friend deny <id>` denies pending requests via `SocialService.denyFriend` → STDB `friend_deny` reducer. `/friend list` now shows each friend's name + online/offline status. `/friend requests` lists each pending request with sender name and accept instructions. `FriendLoginListener` notifies online friends when a player joins. `/party chat <message>` broadcasts to all online party members. `/party info` lists member names with leader/member role tags. `OfflineRewardService` tracks daily login streaks via `SocialService.recordLogin` → STDB `login_streak_record` reducer + `login_streaks` table; streak count displayed on join when > 1 day. `LoginStreak` record holds player UUID, last login timestamp, and current streak. All new message keys configurable in `messages.yml`. Social commands (`/friend add|remove`, `/party invite`, `/mail send`) have `@Complete({"players"})` for tab completion. `StdbPersistenceGateway` hydrates `login_streaks` table (11 tables total) with `toLoginStreak` DTO conversion. 9 SocialService tests cover deny, streak increment/reset/same-day, and gateway persistence. 2 GrantService tests cover `/cleargrants` (clear all grants for target) and `/cgrant`-style grant with duration and reason.

- ✅ **Network cosmetics economy** — Full cosmetic economy wired into `mythic-core`. `CosmeticCatalogLoader` reads `cosmetics.yml` (10 sample items across 6 types with rarity/tradable/limited flags) into `CosmeticManager.register`. `CosmeticService` handles equip/unequip (persisted via `PersistenceGateway.cosmeticEquip`), withdraw (creates PDC-tagged `MythicItem` for trade/sale), redeem (reads PDC `cosmetic_id` on right-click, grants ownership, consumes item), and purchase (deducts via `EconomyManager.withdraw` then grants). `CrateService` loads weighted crate definitions from `crates.yml` (3 tiers: common/rare/legendary with currency costs in COINS/GEMS), `roll()` performs weighted random selection + economy deduction + grant + audit log via `CrateRoll` records. `CosmeticMenuService` provides 5 menu screens: main (type filter tiles + crates button), browse (paginated by type, owned indicators), detail (equip/withdraw actions), crates (paginated crate list), and crate confirm (cost display + confirm/cancel). `CosmeticRedeemListener` handles `PlayerInteractEvent` right-click for PDC-tagged cosmetic items. `/cosmetics` command registered via `CosmeticsCommand`. All menu text YAML-backed via `CosmeticMenuText` from `menus.yml` `cosmetics.*` subtree. STDB persistence extended: `PersistenceGateway.cosmeticEquip` added to interface chain (Noop/Capturing/Stdb); `StdbPersistenceGateway.hydrate()` now subscribes to `COSMETIC_GRANTS` + `COSMETIC_EQUIPPED` (13 tables total); `CoreHydrationSink` + `MainThreadHydrationSink` delegate `applyCosmeticGrant`/`applyCosmeticEquip`. `CosmeticManager.Cosmetic` record extended with `rarity`, `tradable`, `limited` fields. 17 new tests: `CosmeticServiceTest` (9), `CrateServiceTest` (5), `CosmeticHydrationTest` (3).
- ✅ **mythic-hub module** — New Maven module `mythic-hub` with `MythicHubPlugin` bootstrap implementing `MythicPlugin`. `SpawnService` loads configurable spawn location from `hub.yml`, teleports players on join via `MythicScheduler.runOnEntity` (Folia-safe), detects void-fall below configurable Y threshold and teleports back to spawn. `SpawnListener` handles `PlayerJoinEvent` (teleport + join message cancel), `PlayerMoveEvent` (void-fall), and `EntityDamageEvent` (damage cancel in hub). `ServerSelectorService` maintains a live server registry via `ConcurrentHashMap` with role-based group definitions from config + health filtering; `ServerSelectorMenu` renders a `PaginatedMenu` grouped by server role showing player count/TPS/status with click-to-transfer via Paper 1.20.5+ Transfer API. `/servers` command registered via `ServerSelectorCommand`. `HubActivityService` provides configurable double-jump (velocity vector with multiplier + Y boost) and launch pads (material-triggered with separate multiplier/Y boost). `HubActivityListener` handles `PlayerToggleFlightEvent` (double-jump for non-creative/spectator), `PlayerMoveEvent` (launch pad detection), and `PlayerJoinEvent` (flight enable). 7 new tests: `ServerSelectorServiceTest` (5), `SpawnServiceTest` (2).
