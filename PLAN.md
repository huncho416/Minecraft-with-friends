...
- ✅ **Rank/grant menu text → YAML** — `RankMenuText` reads operator-overridable strings from `menus.yml` (`rank.*` subtree): grant flow titles, click hints, confirm/cancel labels, editor section headers, plus configurable `duration-presets` and `reason-presets` lists. `GrantFlowService` and `RankEditorCommand` take the bundle via constructor; old constructors fall back to `RankMenuText.DEFAULTS`. Complete rank editor menu surfaces (full edit-in-game) still pending.
