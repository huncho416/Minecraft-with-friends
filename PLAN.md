...
- ✅ **Friends, party, mail, offline rewards** — `/friend deny <id>` denies pending requests via `SocialService.denyFriend` → STDB `friend_deny` reducer. `/friend list` now shows each friend's name + online/offline status. `/friend requests` lists each pending request with sender name and accept instructions. `FriendLoginListener` notifies online friends when a player joins. `/party chat <message>` broadcasts to all online party members. `/party info` lists member names with leader/member role tags. `OfflineRewardService` tracks daily login streaks via `SocialService.recordLogin` → STDB `login_streak_record` reducer + `login_streaks` table; streak count displayed on join when > 1 day. `LoginStreak` record holds player UUID, last login timestamp, and current streak. All new message keys configurable in `messages.yml`. 9 SocialService tests cover deny, streak increment/reset/same-day, and gateway persistence.
