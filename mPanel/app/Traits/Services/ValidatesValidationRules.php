    ...
    public function validateRules(array|string $rules): void
    {
        try {
            ...
            if (preg_match('/Method \[(.+)\] does not exist\./', $exception->getMessage(), $matches)) {
                throw new BadValidationRuleException(trans('exceptions.nest.variables.bad_validation_rule', ['rule' => Str::snake(Str::replace('validate', '', array_get($matches, 1, 'unknownRule')))]), $exception);
            }
